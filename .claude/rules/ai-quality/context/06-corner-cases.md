...
CORNER_CASES_RULE:
  ...
  format: "Table with input → expected output"
...
| Unicode | "日本語🎉" | Accept (valid UTF-8) | Low |
...
### System Errors
- DB unavailable → Retry with message
- Network timeout → Graceful fallback

### Concurrency
- Double submit → Idempotent handling
```
...
```
□ All input types have null/empty cases
□ All numeric fields have boundary cases
□ All string fields have length/format cases
□ Concurrency scenarios documented
□ External dependency failures covered
□ Invalid state transitions listed
□ Security attack vectors considered
□ Each corner case has expected behavior
□ Priority assigned to each case
```
