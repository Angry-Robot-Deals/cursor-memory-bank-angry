...
**CRITICAL REQUIREMENT:** Before any Level 1 implementation work begins, the agent MUST query MCP servers and include their results in the request context.

### MCP Server Queries Required:
1. **sys8 MCP:**
   - `get_current_datetime` - For fix timestamps and progress tracking
   - `get_os_version` - For platform-specific command execution and path handling

2. **context7 MCP:**
   - `resolve-library-id` - Resolve library names to Context7 library IDs (if libraries are used)
   - `get-library-docs` - Get up-to-date API documentation (if libraries are used)

### MCP Context Integration:
- All MCP query results MUST be included in the request context
- System information from sys8 should inform command execution
- Library documentation from context7 should guide accurate API usage

## 🔧 LEVEL 1 PROCESS FLOW

```mermaid
graph TD
    Start["START LEVEL 1<br>QUICK FIX"] --> MCPGate["⛔ MCP SERVERS<br>CONTEXT GATE"]
    ...
    IncludeContext --> Analyze["1️⃣ ANALYZE<br>Understand issue"]
