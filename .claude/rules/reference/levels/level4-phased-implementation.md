...
## 🔌 MANDATORY MCP SERVER CONTEXT INTEGRATION

**CRITICAL REQUIREMENT:** Before any Level 4 implementation work begins, the agent MUST query MCP servers and include their results in the request context.

### MCP Server Queries Required:
1. **sys8 MCP:**
   - `get_current_datetime` - For phase timestamps, scheduling, and progress tracking
   - `get_os_version` - For platform-specific implementation, command execution, and path handling

2. **context7 MCP:**
   - `resolve-library-id` - Resolve all library names to Context7 library IDs
   - `get-library-docs` - Get up-to-date API documentation for all libraries/frameworks used in implementation

### MCP Context Integration:
- All MCP query results MUST be included in the request context at the start of each phase
- System information from sys8 should inform platform-specific implementation decisions
- Library documentation from context7 should guide accurate API usage throughout all phases
...
flowchart TD
    ...
    classDef mcp fill:#ff5555,stroke:#dd3333,color:#fff
    
    Start([Begin Implementation<br>Process]) --> MCPGate[⛔ MCP SERVERS<br>CONTEXT GATE]
    ...
    IncludeContext --> Framework[Establish Implementation<br>Framework]
    ...
    class MCPGate,QuerySys8,QueryContext7,IncludeContext mcp
...
- sys8 MCP queries completed? [YES/NO]
- context7 MCP queries completed? [YES/NO]
