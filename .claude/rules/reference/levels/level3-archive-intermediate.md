...
graph TD
    ...
    CreateArchiveFile["3. Create Feature Archive File<br>e.g., `memory-bank/archive/archive-[task_id].md`"] -->
...
## 📝 Structure for `memory-bank/archive/archive-[task_id].md`

  * **Feature Title:** (e.g., "Archive: User Profile Feature - Avatar Upload Enhancement")
  * **Feature ID (from `tasks.md`):**
  * **Date Archived:** YYYY-MM-DD (MUST use sys8 MCP `get_current_datetime` - use `date` field from response)
