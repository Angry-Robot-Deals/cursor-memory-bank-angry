...
// ✅ SAFE: Use SERIALIZABLE or UNIQUE constraint
await tx.query('SET TRANSACTION ISOLATION LEVEL SERIALIZABLE');
...
```typescript
// ❌ RACE CONDITION: Lost increments
...
```markdown
□ Identified operations that need transactions
□ Determined appropriate isolation level
□ Documented isolation choice in requirements
□ Planned unique constraints if needed
```

### During Implementation

```markdown
□ SET TRANSACTION ISOLATION LEVEL explicit
□ Added unique constraints where needed
□ Used FOR UPDATE for read-modify-write
...
```markdown
□ Wrote concurrent operation test
□ Tested 100+ simultaneous requests
□ Verified final state is correct
□ Tested timeout/failure scenarios
```
...
```
TRANSACTION RULES:
├── Always SET ISOLATION explicitly
├── Add UNIQUE constraints for dedup
├── Use FOR UPDATE for read-modify-write
├── Use atomic UPDATE for counters
└── Test with 100+ concurrent requests

ISOLATION SELECTION:
├── Simple CRUD → READ COMMITTED
├── Multiple reads → REPEATABLE READ
├── Critical ops → SERIALIZABLE
└── When in doubt → SERIALIZABLE
```
