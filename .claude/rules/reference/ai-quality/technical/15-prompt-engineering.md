...
**Structure prompts with: Context → Task → Expected Results → DoD → Boundaries.**
...
- Null input → throw 'Input required'
- Empty string → throw 'Input cannot be empty'
- String > 1000 chars → throw 'Input too long'"
```
...
- 'not-an-email' → invalid: 'Invalid email format'
...
3. Examples (concrete input→output)
...
```
□ Task is specific and single-focused
□ Context is minimal but sufficient
□ Examples are concrete (input → output)
□ DoD is testable
□ Boundaries are explicit
□ Total length < 500 words
□ I can verify if response is correct
□ No vague words ("better", "proper", "good")
```
