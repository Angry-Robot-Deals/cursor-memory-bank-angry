...
**Maximum 7±2 objects in method scope. Exceeding this limit causes quality degradation.**
...
```yaml
COGNITIVE_LOAD_RULE:
  max_objects: 9  # Upper limit of 7±2
  ideal_objects: 5-7
  warning_threshold: 8
...
──────────────────────────────────────────────────
  1-5      100%     ✅ Ideal      Perfect scope
  6-7       95%     ✅ Good       Optimal range
  8-9       85%     ⚠️ Warning    Consider simplifying
 10-12      70%     ❌ Problem    Refactor needed
 13-15      50%     ❌ Critical   Immediate refactor
  15+       30%     ❌ Broken     Split function
```
...
// ✅ GOOD: 1 config object, destructure only needed
...
```
1. □ Can parameters be grouped into config?
2. □ Can logic be extracted to helpers?
3. □ Can early returns reduce nesting?
4. □ Are all variables necessary?
5. □ Can scope be narrowed?
6. □ Should function be split?
```
...
├── Objects > 9: Split function
├── Params > 5: Use config
├── Nesting > 3: Early returns
└── Lines > 50: Extract methods
```
