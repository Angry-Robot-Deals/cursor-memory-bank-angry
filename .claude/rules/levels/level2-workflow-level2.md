...
```mermaid
graph TD
    Start["Begin Task<br>Planning"] --> MCPGate["⛔ MCP SERVERS<br>CONTEXT GATE"]
    ...
    IncludeContext --> Requirements["Define Clear<br>Requirements"]
...
**Steps:**
1. **MANDATORY:** Query sys8 MCP (get_current_datetime, get_os_version) and context7 MCP (resolve-library-id, get-library-docs)
2. **MANDATORY:** Include all MCP results in request context
3. Define clear requirements
4. Break down into subtasks
5. Update tasks.md with subtasks
6. Create time estimates
...
- MCP results included in context? [YES/NO]
