...
**CRITICAL REQUIREMENT:** Before any Level 3 planning work begins, the agent MUST query MCP servers and include their results in the request context.

### MCP Server Queries Required:
1. **sys8 MCP:**
   - `get_current_datetime` - For planning timelines and scheduling
   - `get_os_version` - For platform-specific planning considerations

2. **context7 MCP:**
   - `resolve-library-id` - Resolve all library names to Context7 library IDs
   - `get-library-docs` - Get up-to-date documentation for all libraries/frameworks

### MCP Context Integration:
- All MCP query results MUST be included in the request context
- System information from sys8 should inform platform-specific decisions
- Library documentation from context7 should guide technology selection and API usage

## 🏗️ PLANNING WORKFLOW

```mermaid
graph TD
    Start["Planning Start"] --> MCPGate["⛔ MCP SERVERS<br>CONTEXT GATE"]
    ...
    IncludeContext --> Req["📋 Requirements<br>Analysis"]
