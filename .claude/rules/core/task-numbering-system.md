...
4. **Extract numbers** from task IDs (e.g., `DEV-0053` → `53`)
...
```
⚠️ WARNING: Task ID [PREFIX]-[NUMBER] already exists
Generating next available number...
→ Assigned: [PREFIX]-[NEXT_NUMBER]
```
...
```
ℹ️ Could not determine task prefix from content
